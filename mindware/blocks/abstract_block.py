...
class AbstractBlock(object):
    def __init__(self, node_list, node_index,
        ...
        if self.task_type in CLS_TASKS and self.task_type not in [TEXT_CLS, IMG_CLS]:
    ...
    def fit_ensemble(self):
        if self.ensemble_method is not None:
            if self.topk_pkl is not None:
                with open(self.topk_pkl, 'rb') as f:
                    stats = pkl.load(f)
            else:
                config_path = os.path.join(self.output_dir, '%s_topk_config.pkl' % self.timestamp)
                with open(config_path, 'rb') as f:
    ...
    def _predict(self, test_data):
        if self.ensemble_method is not None:
            ...
            elif self.es is None and self.topk_pkl is not None:
                self.fit_ensemble()
            ...
            if self.task_type in CLS_TASKS:
                if self.task_type in [TEXT_CLS, IMG_CLS]:
                    return estimator.predict_proba(test_data_node, self.metric, test=True)
                else:
    ...
    def score(self, test_data, metric_func=None):
