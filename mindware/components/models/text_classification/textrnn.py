...
class TextRNNModel(nn.Module):
    ...
    def __init__(self, vocab_size, embed_size, hidden_size, layer, bidirection, drop_rate, output_dim, device='cuda'):
    ...
    def forward(self, x_in):
        ...
        h0 = torch.zeros(self.layer * 2, x.size(0), self.hidden_size) if self.bidirection else torch.zeros(self.layer, x.size(0), self.hidden_size)
        c0 = torch.zeros(self.layer * 2, x.size(0), self.hidden_size) if self.bidirection else torch.zeros(self.layer, x.size(0), self.hidden_size)
...
class TextRNN(BaseTextClassificationNeuralNetwork):
    ...
    def __init__(self, embed_size=100, hidden_size=10, layer=2, bidirection=False, drop_rate=0.5,
                 ...
                 dl_model_path='/tmp/'):
        super(TextRNN, self).__init__(optimizer, batch_size, epoch_num, lr_decay, weight_decay,
    ...
    def fit(self, dataset: TotalTextDataset, scorer):
        ...
        self.model = TextRNNModel(vocab_size=self.vocab_size, embed_size=self.embed_size,
                                  hidden_size=self.hidden_size, layer=self.layer,
                                  bidirection=self.bidirection, drop_rate=self.drop_rate,
                                  output_dim=self.output_dim, device=self.device)
